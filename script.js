...
/* Story Builder V1.50 script.js - Part 2/3 */
    ...
    window.deleteCurrentChapter=async()=>{if(window.currentChapterId&&confirm("削除しますか？")){await db.collection('works').doc(window.currentWorkId).collection('chapters').doc(window.currentChapterId).delete();window.currentChapterId=null;document.getElementById('main-editor').value="";showMobileChapterList();loadChapters();}};
    window.deleteTargetChapter=async(id)=>{if(confirm("削除しますか？")){await db.collection('works').doc(window.currentWorkId).collection('chapters').doc(id).delete();loadChapters();}};
...
/* Story Builder V1.50 script.js - Part 3/3 */
    ...
    window.loadDailyLog = async function() {
        ...
        const st=document.getElementById('stat-today'); if(st) st.innerHTML=`${window.todayAddedCount}<span class="unit">字</span>`;
        const sw=document.getElementById('stat-week'); if(sw) sw.innerHTML=`${w}<span class="unit">字</span>`;
    };
